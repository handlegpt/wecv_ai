...
export const verifyPermission = async (
  ...
  // 检查当前权限
  if ('queryPermission' in handle && (await (handle as any).queryPermission(options)) === "granted") {
    return true;
  }

  // 请求权限
  if ('requestPermission' in handle && (await (handle as any).requestPermission(options)) === "granted") {
    return true;
  }
