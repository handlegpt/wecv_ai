  ...
  useEffect(() => {
    ...
    try {
      if (resumeId && resumes[resumeId]) {
        ...
        setIsLoading(false);
      } else if (resumeId && Object.keys(resumes).length > 0) {
        // 如果简历不存在但resumes已经加载，说明简历确实不存在
        // 不设置activeResume，保持当前状态
        setIsLoading(false);
      } else if (resumeId) {
        // 如果resumes还没有加载完成，继续等待
        setIsLoading(true);
      }
    } catch (error) {
      setIsLoading(false);
    }
  }, [resumeId, resumes, setActiveResume]);
